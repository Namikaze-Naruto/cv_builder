 ...
 * Style: Clean academic CV. Long form. Centered name. Tight margins. Section titles with ruled lines.
 * Inspired by: Harvard/MIT/Oxford academic CV format.
 */
...
const T6Section = ({ section }) => {
    ...
    const renderContent = () => {
            ...
            <div key={item.id} className="mb-4">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <span className="text-[0.78rem] text-gray-700">{item.company && `, ${item.company}`}{item.location && `, ${item.location}`}</span>
                    </div>
        ...
        if (section.type === 'publications') return items.map(item => (
            <p key={item.id} className="mb-2 text-[0.75rem] text-gray-700 leading-relaxed">
                {item.description || `${item.title || ''} ${item.year || ''}`}
            </p>
        ));
