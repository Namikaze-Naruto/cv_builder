...
const Residency = ({ items, className = '' }) => {
            ...
            {items.map((item) => (
                <div key={item.id} className="flex flex-col">
                    ...
                    <div className="flex justify-between items-baseline mb-2 text-sm">
                        <span className="font-semibold text-primary-700">{item.hospital || item.company}</span>
                        {item.location && <span className="text-gray-500 italic shrink-0 ml-4">{item.location}</span>}
                    </div>
                    {item.specialty && (
                        <div className="text-xs text-gray-500 mb-1">Specialty: {item.specialty}</div>
                    )}
                    {item.description && (
